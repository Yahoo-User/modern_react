...
// -----------------------------------------------------------------
// What if "T": ValidatableInputMethods2, "P": ReactInputProps ???
// -----------------------------------------------------------------
// This,
...
export
const ValidatableInput2 = forwardRef<ValidatableInputMethods2, ReactInputProps>(
    ({ className: _className, ...inputProps }, methodsRef) => {                             // <--- ***
        ...
        */
        useEffect(() => {
            console.log(`☞ [ ValidatableInput2 ] Component is Mounted or Re-rendered(${++renderCount}) <=====================.`);
        })

        // -----------------------
        console.group("●●● (src/theme/ValidatableInput2) ●●●")
        console.groupEnd()
        ...
        useImperativeHandle(methodsRef, () => ({                                            // <--- ***
            ...
            validate: () => {                                                               // <--- ***
                const valid = true
                //                   ----------- *** ------------
                return valid? [true, "The 'value' prop of <input>"] : [false, "Wrong email adderess"]
            } // validate
        }), [])                                     // <--- ***                                   // <--- ***
